...
3. Update architecture or setup documentation when behavior, configuration, or developer workflow changes.
4. Run the relevant tests.
5. Run `pre-commit run --all-files`.
6. Commit with a conventional commit message.
7. Push the branch.
8. Open a pull request with a closing keyword such as `Closes #X`.
9. Wait for CI to pass and fix any failures before merge.
10. Review the diff, address feedback, and create `[followup]` issues for deferred work when needed.
11. Merge the pull request.
12. Return to the primary checkout, pull `origin/main`, update today's session note, then start the next issue.
...
- Treat CI, workflow, release, deployment, and setup-documentation regressions as high severity.
